...
from catabra_pandas import make_windows, resample_eav, resample_interval
...
def test_groupby():
    ...
    mw = make_windows(df="df.groupby(entity_col)[time_col].min().to_frame()", start=start, stop=stop)
    windows = mw(df=_df, entity_col="entity", time_col="start")
...
def test_df():
    ...
    mw = make_windows(df=df, entity="subject_id", start_rel=start_rel, anchor="timestamp")
...
def test_invalid_args():
    ...
    mw = make_windows(
        entity=pd.Series([1, 2, 3], index=[0, 0, 1]), start=pd.Series(pd.Timestamp.now(), index=[1, 0, 0])
    )
    try:
        mw(df=_df, entity_col="entity", time_col="start")
    except ValueError:
    ...
    mw = make_windows(
        entity=pd.Series([1, 2, 3], index=[0, 1, 2]), start=pd.Series(pd.Timestamp.now(), index=[1, 2, 3])
    )
    try:
        mw(df=_df, entity_col="entity", time_col="start")
    except ValueError:
