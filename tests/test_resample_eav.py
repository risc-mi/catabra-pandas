...
from catabra_pandas import resample_eav
...
def test_corner_cases():
    df = pd.DataFrame(
        data=dict(
            ...
            value=[np.nan, 85.7, 151.4, 137.8, np.nan, 49.0, 50.0, 50.0, 0.1, 0.2, 38.7, 36.1],
        )
    )

    # no data, only NaN values, both "attr_3" values, only one "attr_3" value
    windows = pd.DataFrame(data=dict(entity=[1000] * 4 + [1001], start=[-1, 0, 1, 7, 0], duration=[1, 8, 40, 10, 2]))
...
@pytest.mark.parametrize(
    ids=["random", "regular_non_overlapping", "regular", "non_overlapping", "infinite", "na_windows"],
    argnames=["pattern", "include_start", "include_stop", "na_windows"],
    argvalues=[
...
def test_windows(pattern: str, include_start: bool, include_stop: bool, na_windows: bool):
    if pattern == "mixed":
        ...
        for p in ["random", "regular", "regular non-overlapping", "non-overlapping"]:
            df1, windows1 = create_random_data(
                2500, 250, attributes=["attr_" + str(i) for i in range(1, 7)], window_pattern=p, seed=seed
            )
            df1["entity"] += i
            windows1[("entity", "")] += i
        ...
        df, windows = create_random_data(
            10000, 100, attributes=["attr_" + str(i) for i in range(1, 7)], window_pattern="random", seed=seed
        )
        windows.drop([("timestamp", "start")], axis=1, inplace=True)
    else:
        df, windows = create_random_data(
            10000, 100, attributes=["attr_" + str(i) for i in range(1, 7)], window_pattern=pattern, seed=seed
        )
...
def test_single_entity_attribute():
    ...
    kwargs = dict(time_col="timestamp", value_col="value", include_start=True, include_stop=True)
    out1 = resample_eav(
        df,
        windows,
        agg,
...
def test_categorical():
    df, windows = create_random_data(
        10000, 100, attributes=["attr_1", "attr_2"], value_dtype="category", window_pattern="random", seed=seed
    )
...
def test_custom_agg(seed=None):
    ...
    kwargs = dict(entity_col="entity", time_col="timestamp", value_col="value", include_start=True, include_stop=False)
...
def test_include_stop():
    df, _ = create_random_data(
        10000,
        1,
        n_entities=200,
        attributes=["attr_" + str(i) for i in range(1, 10)],
...
def test_large(dask):
    ...
    df, _ = create_random_data(
        ...
        1,
        n_entities=20000,
        attributes=["attr_" + str(i) for i in range(1, 50)],
        window_pattern="random",
        time_dtype="timestamp",
        seed=seed,
    )
    ...
    if not dask:
        ...
        out_ddf2 = resample_eav(
            ddf, dd.from_pandas(windows.reset_index(drop=True), npartitions=8), agg, optimize="time", **kwargs
        )
