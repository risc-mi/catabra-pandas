...
from catabra_pandas import prev_next_values
...
def test_no_group(seed=None):
    ...
    out = prev_next_values(
        df,
        sort_by="timestamp",
        columns={"value": dict(prev_name="value_prev", next_name="value_next")},
...
def test_all_columns(seed=None):
    df, _ = create_random_data(
        100000, 1, n_entities=500, attributes=["attr_1", "attr_2", "attr_3"], seed=seed, time_dtype="timestamp"
    )
    ...
    out = prev_next_values(
        ...
        columns={"value": dict(prev_name="value_prev"), "timestamp": dict(prev_name="timestamp_prev")},
        first_indicator_name="is_first",
        inplace=False,
        keep_sorted=True,
    )
...
def test_mixed_index_columns(seed=None):
    df, _ = create_random_data(
        100000, 1, n_entities=100, attributes=[f"attr_{i}" for i in range(10)], seed=seed, time_dtype="float"
    )
    ...
    out = prev_next_values(
        ...
        columns={"value": dict(next_name="value_next")},
...
def test_integer(seed=None):
    ...
    out = prev_next_values(
        ...
        sort_by="timestamp",
        group_by=0,
        columns={"value": dict(prev_name="value_prev", prev_fill=0)},
        first_indicator_name="is_first",
        last_indicator_name="is_last",
        inplace=True,
        keep_sorted=True,
...
def test_na(seed=None):
    ...
    orig_columns = df.columns.copy()
    out = prev_next_values(
        df, sort_by="timestamp", group_by="entity", columns={"timestamp": dict(next_name="timestamp_next")}
    )
    _check_result_partial(df, orig_index, orig_columns)
    assert (df.index == orig_index).all()
    assert (df.columns == orig_columns).all()
...
def test_multi_columns(seed=None):
    ...
    out = prev_next_values(
        ...
        sort_by=("timestamp", ""),
        group_by=[("entity", ""), ("attribute", "")],
        columns={("value", ""): dict(prev_name="prev", next_name="next")},
    )
    _check_result_partial(df, orig_index, orig_columns)
    assert (df.index == orig_index).all()
    assert (df.columns == orig_columns).all()
...
def test_large(seed=None):
    ...
    out = prev_next_values(
        ...
        sort_by=0,
        group_by=["entity", "attribute"],
        columns={"value": dict(prev_name="value_prev"), "entity": dict(next_name="entity_next", next_fill=-1)},
    )
    _check_result_partial(df, orig_index, orig_columns)
    assert (df.index == orig_index).all()
    assert (df.columns == orig_columns).all()
