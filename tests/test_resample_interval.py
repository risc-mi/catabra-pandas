...
from catabra_pandas import resample_interval
...
@pytest.mark.parametrize(
    ...
    argnames=["pattern", "include_df_start", "include_df_stop", "na_windows"],
    argvalues=[
    ...
    if pattern == "mixed":
        ...
        for p in ["random", "regular", "regular non-overlapping", "non-overlapping"]:
            df1, windows1 = create_random_data(
                2500,
                250,
                attributes=["attr_" + str(i) for i in range(1, 7)],
                intervals=True,
                window_pattern=p,
                seed=seed,
            )
...
def test_single_entity_attribute():
    df, windows = create_random_data(
        10000, 100, n_entities=1, intervals=True, window_pattern="random", value_dtype="int", seed=seed
    )

    kwargs = dict(start_col="start", stop_col="stop", time_col="timestamp", value_col="value")
    out1 = resample_interval(df, windows, **kwargs)
    ground_truth = resample_interval_slow(df, windows, **kwargs)
    compare_dataframes(out1, ground_truth)
