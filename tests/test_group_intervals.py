...
from catabra_pandas import group_intervals
...
def test_no_group():
    ...
    grp = group_intervals(
        df, start_col="start", stop_col="stop", distance=(df["start"].max() - df["start"].min()) * 0.01
    )
...
def test_integer():
    ...
    orig_columns = df.columns.copy()
    grp = group_intervals(
        df, group_by=0, point_col="timestamp", distance=(df["timestamp"].max() - df["timestamp"].min()) * 0.005
    )
    _check_result(grp, df, orig_index, orig_columns)
...
def test_na():
    ...
    orig_columns = df.columns.copy()
    grp = group_intervals(
        df, group_by="entity", start_col="start", stop_col="stop", distance=(df["stop"].max() - df["stop"].min()) * 0.1
    )
...
def test_multi_columns():
    ...
    df.columns = pd.MultiIndex.from_tuples(
        [(c, "" if c in ("entity", "attribute", "timestamp") else str(i)) for i, c in enumerate(df.columns)]
    )
