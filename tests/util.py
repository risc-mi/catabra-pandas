...
def create_random_data(
    ...
    time_dtype: str = "timedelta",
    value_dtype: str = "float",
    intervals: bool = False,
    window_pattern: str = "random",
    ...
    if value_dtype in ("float", "timedelta", "timestamp", "category", "str"):
        ...
        values.iloc[perm[: int(np.power(10.0, rng.uniform(-1, np.log10(n_observations / 2))))]] = None
    if intervals:
        duration = np.power(20.0, -rng.uniform(0, 1, size=n_observations))
        duration[rng.choice([False, True], size=n_observations, replace=True, p=[0.67, 0.33])] = 0.0
    ...
    if n_entities is None:
        n_entities = max(2, int(np.round(np.power(10.0, rng.uniform(0, np.log10(n_windows))))))
    if n_entities > 1:
    ...
    duration = np.power(10.0, -rng.uniform(0, 1, size=n_windows)) * (max_ts - min_ts)
    ...
    if dtype == "float":
        ...
        return pd.Series(
            data=pd.Categorical.from_codes(
                rng.choice(m, size=n, p=p, replace=True), categories=["cat_" + str(i) for i in range(m)], ordered=True
            )
        )
    else:
        assert False
